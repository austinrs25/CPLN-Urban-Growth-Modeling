...
We assume a new primary highway is introduced in southeast Mecklenburg County, connecting US Route 521 to Billy Graham Parkway, to enhance commuting access and connection to the city. This infrastructure investment is expected to increase the likelihood of development in surrounding areas. Distance to this corridor is incorporated as a predictor variable in both the t2 model and the t3 forecast.

## 3.1. Setup Libraries, Functions, and Color Palette
...
**Reclassify Land Cover as Developed and Undeveloped:**
...
**Detect Change from 2011 (t1) to 2019 (t2):**
...
For both t1 (2011) and t2 (2019), we construct binary raster layers representing the presence of each land use type. These rasters are named accordingly and compiled into lists.
...
TO DO: Include the hypothetical transport features for t3 forecast
COMMENT: I think we’ll need to revise this step below (both the code and narrative) to reflect the inclusion of the new highway in the t3 forecast - I could be wrong that it is not this step though. Refer to Section 7.2 in Michael’s RMD for guidance. The best approach is likely to duplicate the current code to represent the "before" scenario, and then modify the copied version to reflect the "after" scenario with the updated highway input. This way, we preserve both versions for comparison.
...
We compute a new variable, `distance_highways_2011`, for each fishnet cell by measuring the straight-line distance to the nearest highway feature. This step uses `st_distance` and applies to centroid points derived from the fishnet. These distances are added back to the fishnet, enabling their use in modeling and spatial analysis. To maintain modeling consistency, we mirror this step for `t2`, creating `distance_highways_2019`. 

The workflow in this section assumes that the highway network remains consistent between 2011 and 2019. In **Section 6**, we will simulate the effect of proposed infrastructure on development forecasts by modifying `distance_highways_2019` to include an additional proposed highway connecting US Route 521 to downtown Charlotte.
...
# 5. Modeling
In this stage, we create models to predict where development is most likely to occur in the Charlotte MSA. Using logistic regression, we explore how well different combinations of predictors can estimate the binary outcome of land cover change (undeveloped to developed) between 2011 and 2019.
...
TO DO: Austin and Abe, review selection of Model 6 and confirm that it’s the best choice for our forecast. This decision impacts rest of Section 5 below. Before determining, take a look at Section 12.2 in Michael’s RMD (note: I’ve split that section (12.2) into 5.2 and 5.3 in our version for clarity).
...
Having validated the accuracy of our model, we now apply it to generate a forecast for urban development in the Charlotte region for the year 2027 using `t2` (2019) data. This forecast incorporates an updated `distance_highways_2019` variable which reflects a proposed new highway extension to US Route 521.
...
**Modify 2019 Data to Include Proposed Highway Extension:**
The proposed highway extension was generated in ArcGIS Pro and exported as a GeoJSON file. This new feature is imported and combined with the existing highway network.

```{r}
#import new highway and join to existing highways dataframe
newHighway <- 
  st_read("https://raw.githubusercontent.com/austinrs25/CPLN-Urban-Growth-Modeling/refs/heads/main/new_charlotteHighways.geojson") 
newHighway <- st_transform(newHighway, st_crs(charlotteHighways))
missing_cols <- setdiff(names(charlotteHighways), names(newHighway))
newHighway[missing_cols] <- NA
updatedcharlotteHighways <- bind_rows(charlotteHighways, newHighway)

# create updated 2019 highway points fishnet
updated_highwayPoints_fishnet_2019 <- charlotteMSA_fishnet %>%
  st_centroid() %>%
  mutate(distance_highways_2019 = as.numeric(st_distance(., st_union(updatedcharlotteHighways) %>% 
                                                           st_transform(st_crs(charlotteMSA_fishnet))))) %>%
  as.data.frame() %>% 
  dplyr::select(-geometry) %>% 
  left_join(charlotteMSA_fishnet, .) %>% 
...
```{r}
#plot updated distance to highways to confirm addition of new highway
ggplot() +
  geom_sf(data=charlotteMSA %>% st_transform(st_crs(updated_highwayPoints_fishnet_2019))) +
  geom_point(data=updated_highwayPoints_fishnet_2019, aes(x=xyC(updated_highwayPoints_fishnet_2019)[,1], 
                                                  y=xyC(updated_highwayPoints_fishnet_2019)[,2], 
                                                  colour=factor(ntile(distance_highways_2019,5))),size=1.5) +
  scale_colour_manual(values = palette5,
                      labels=substr(quintileBreaks(updated_highwayPoints_fishnet_2019,"distance_highways_2019"),1,8),
                      name="Quintile\nBreaks") +
  geom_sf(data=updatedcharlotteHighways, colour = "red") +
  labs(title = "Updated distance to Highways (m)",
       subtitle = "As fishnet centroids; Highways visualized in red") +
  theme_void()
```

```{r}
#updated dat_2019 for modeling
dat_2019_updated <- 
  cbind( fishnet, updated_highwayPoints_fishnet_2019, fishnetPopulation19, lcRasters_2019, countyFishnet) %>%
  as.data.frame() %>%
  dplyr::select(uniqueID, development_change, lagDevelopment_2019, distance_highways_2019, pop_2019,  
                developed_2019, forest_2019, farm_2019, wetlands_2019, otherUndeveloped_2019, water_2019,
                NAME, geometry) %>%
...
```{r}
dat_2027_preds <- dat_2019_updated %>%
    mutate(probs = predict(Model6, dat_2019_updated, type="response") ,
           Prediction = as.factor(ifelse(probs >= 0.17 ,1,0))) %>%
  st_as_sf()

```

**Visualize 2027 Predictions:** Visualize the forecast using a spatial plot, identifying areas across Mecklenburg, Cabarrus, Gaston, and Union counties that are most at risk of development pressure.
...
ggplot(data=dat_2027_preds) +
  ...
  labs(title="Forecast 2: Development Predictions, 2027") + 
  theme_void()
```
...
With the 2027 development forecast complete, we now assess the scale and distribution of predicted growth across the Charlotte Metropolitan Area. This step is essential for planners to anticipate infrastructure needs, protect sensitive lands, and align policies with projected development demand.
...
DELIVERABLE: "Assessment of impact - where is development likely to occur? Are sensitive lands at risk? Where?"

This impact assessment provides the quantitative foundation needed to interpret the spatial and environmental implications of the proposed highway and forecasted development in the Charlotte MSA between 2019 and 2027. 

The most notable prediction of this model is that **forested land** in Mecklenburg and Cabarrus counties is by far most likely to develop within the next decade. Tree canopy loss is a major concern for cities experiencing rapid development. Reduction in forested areas can negatively impact air quality and wildlife habitats, as well as contribute to the "urban heat island" effect.

One additional concern raised by the model predictions is the high likelihood of development along the northwest border of Mecklenburg County, adjacent to the Catawba River. The Catawba River basin represents the primary water supply for the Charlotte MSA, and increased development alongside the river may increase the risk of water contamination from runoff.
...
**DELIVERABLE:** "Key recommendations - use your planning expertise acquired through readings and lectures - give three key recommendations about growth allocation and management implications given the results of your forecast. This requires you to do some desk research about population and employment forecasts for your region (these are published by local MPOs) or about important local growth management issues."

**NOTE:** Reference Step 15 in Michael's RMD file.

Based on the results of our urban growth model, we have developed the following recommendations to help manage urban growth in the Charlotte MSA over the next decade:

1. Incentivize development of green stormwater infrastructure

Aging stormwater infrastructure is a known problem in the Charlotte MSA. With the city's rapidly growing population, stormwater management is all the more important to reduce the risk of combined sewer overflows during storm events. As Charlotte seeks to modernize its stormwater infrastructure, green infrastructure projects such as rain gardens should be incorporated to reduce strain on existing drainage systems. This type of infrastructure can also help offset the worsening of the urban heat island effect caused by tree canopy loss.

2. Expand the light rail system into Cabarrus County

Between 2010 and 2022, the population of Cabarrus County increased by 32.1%. Our model predicts that continued development in the suburbs north of Charlotte in Cabarrus County is likely to occur. However, the Charlotte Area Transit System (CATS) light rail does not extend into Cabarrus County. The Amtrak system is currently the only way to travel by rail between the Cabarrus County suburbs and downtown Charlotte. This increases reliance on the highway system and incentivizes sprawl. Extending the CATS Blue Line into Cabarrus County could help improve commuter traffic into Charlotte and reduce sprawl by concentrating development along the rail line.

3. Adjust zoning regulations to favor higher-density zoning within Mecklenburg County

In our model, the majority of cells predicted to convert from undeveloped to developed between 2019 and 2027 are forested areas in Mecklenburg County (77/138 fishnet cells). As such, upzoning in Mecklenburg County is recommended to help accommodate population growth while limiting the destruction of forested areas.

~some kind of cool last sentence that ties it all together? I don't think we need a conclusion section per se~
...
1. ALERT 1: **Create the hypothetical transport features** for t3 forecast and **include where applicable** (Steps 3.4 AND ???)

2. ALERT 2: In Step 5, **confirm Model 6 is best fit**

3. ALERT 3: **Revise Steps 6 and on** 

4. ALERT 4: In Step 7, **write narrative imapcts assessment** based on 2027 forecast results### Assessment of impact - where is development likely to occur? Are sensitive lands at risk? Where?

5. ALERT 5: **Draft Recommendations for Charlotte:**

6. **Discuss on Wedensday**
-   Do we need Conclusion section?
-   Confirm we have met **Deliverables**
-   Check we have fulfilled **Rubric**
-   Both separately run code from start to finish
-   Both separately read final RMD file
-   Rename and retitle file so its not the same as Michael's
-   **Export final RMD file** to HTML and **Upload to Canvas**
