...
# I BELIEVE THIS STEP (CODE AND NARRATIVE TEXT) WILL NEED TO BE ADJUSTED TO INCLUDE NEW HIGHWAY FOR T3 FORECAST  -- **REFERENCE 7.2 IN MICHAEL'S RMD** -- CREATE A BEFORE AND AFTER, SO LIKELY COPY AND PASTE AND ONLY ADJUST THE PASTED CODE SO THE CURRENT CODE CAN STAY AND REPRESENT THE BEFORE AND YOUR PASTED AND THEN EDITED CODE WILL REPRESENT THE AFTER
...
# REMOVE THIS LINE AND MY COMMENT LINE WHEN COMPLETE
...
#AUSTIN AND ABE REVIEW SLECTION OF **MODEL 6** AND CONFIRM IT IS BEST - AFFECTS 5.3 and 5.4 BELOW -- READ 12.2 IN MICHAELS RMD BEFORE SELECTING (I SPLIT 12.2 into two as 5.2 and 5.3)
...
To evaluate the performance of our selected model **(Model 6)**, we begin by applying it to our test dataset. The model returns predicted probabilities for each cell, indicating the likelihood of development between 2011 and 2019.
...
### 5.4.1 Threshold Selection and Error Analysis

Since threshold choice affects model sensitivity and specificity, we explore the implications of two thresholds: 5% and 17%. A lower threshold increases sensitivity (detecting more change), while a higher threshold improves specificity (avoiding false positives).

To understand where the model performs well—or falls short—we classify each cell in the t2 dataset based on the prediction outcomes at both thresholds. We summarize results by county and visualize spatial distributions of classification results. These maps and summaries help us understand how predictive performance varies spatially across the Charlotte MSA and guide recommendations for planning interventions.

We validate Model 6 using the test set by calculating predicted probabilities and generating a confusion matrix with a 5% classification threshold. This allows us to quantify true positives, false positives, and other prediction outcomes. We also visualize model performance using a ROC curve, confirming that the model distinguishes well between developed and undeveloped cells.



## 5.5 Threshold Testing and Spatial Error Analysis

To assess the impact of different thresholds, we compare results using 5% and 17% probability cutoffs. These thresholds reflect trade-offs between capturing more true positives versus minimizing false positives. We classify cells accordingly and generate confusion matrix outcomes.

Finally, we evaluate threshold-specific prediction accuracy across counties. Visualizing error categories spatially helps reveal geographic variations in model performance and potential biases. This step is critical in understanding where the model may over- or under-predict development and guides interpretation of the final forecast.
