...
Charlotte, North Carolina, is one of the fastest-growing metro areas in the southeastern United States. It's population grew by 35.2% from 2000 to 2010 and 19.6% from 2010 to 2020, and its forecasted to grow an additional 14.1% from 2020 to 2030. This project aims to forecast urban development in the Charlotte Metropolitan Area, specifically Mecklenburg, Cabarrus, Gaston, and Union counties, by the year 2031. The forecast is based on land cover change from 2011 to 2021 and uses a logistic regression model to project future development patterns. **This scenario includes a proposed --- new highway running from southeastern Mecklenburg into downtown Charlotte, designed to increase connectivity and redirect growth pressure. --- **

Our objective is to support proactive growth management by identifying areas at risk for sprawl, encroachment on environmentally sensitive lands, and potential conflicts with existing infrastructure or planning goals. The Charlotte region presents a valuable case study due to its ongoing growth.

## 1.2. Data Sources

We obtained land cover data from the Multi-Resolution Land Characteristics (MRLC) consortium (https://www.mrlc.gov/data?f%5B0%5D=category%3ALand%20Cover), covering 2011 and 2019. Census data was sourced from the United States Census Bureau (https://api.census.gov/data/key_signup.html). Roads data was sourced from data.gov and downloaded using the `Tigris` package (https://catalog.data.gov/dataset/tiger-line-shapefile-2023-state-north-carolina-primary-and-secondary-roads). County Boundary data was sourced from North Carolina Department of Transportation (https://www.nconemap.gov/datasets/NCDOT::ncdot-county-boundaries/about). 

# 2. Exploratory Analysis and Feature Engineering

**Provide narrative overview of *Exploratory Analysis and Feature Engineering* here**

## 2.1. Setup - Load Libraries, Functions, and Color Pallette

The packages used for our model and analysys are below. A color palette used to visualize the analyis is also provided below. 

**The packages are as follows:**

- `tidyverse` - main data wrangling and visualization packages including `dplyr` and `ggplot2`

- `sf` - core R-based vector GIS package

- `raster` - raster GIS package - this package doesn't always use the "tidy" syntax we are used to. `terra` is the main competitor for raster work in R - if you don't like `raster`, try it out.

- `kableExtra` - package that makes nice readable tables in R Markdown

- `tidycensus` - package for calling census data from the US Census API

- `tigris` - library for calling US Census Bureau vector GIS data

- `FNN` - spatial point-pattern analysis package that we use for K-nearest neighbor

- `caret` - package used for applied predictive modeling

- `yardstick` - package for summarizing model performance

- `plotROC` - tool to plot ROC curves, a diagnostic for logistic regression

- `ggrepel` - graphics package to handle labels in `ggplot2`

- `pROC` - package to analyze ROC curves

- `grid` - graphics package to display multiple plots in one visualization

- `gridExtra` - package used to make small multiple arrangements of ggplots

- `viridis` - library with nice color ramps for our charts and maps.

- `igraph` - package for network analysis

- `mapview` - quick web mapping package that can handle sf and raster data

- `FedData` - package for accessing federal satellite imagery via API
...
**Several custom built functions are used in this workflow:** 
...
## 2.2. Setup - Load the Study Area Boundary

Our study area is the Charlotte MSA. A shapefile for the study area was engineered in ArcGIS Pro and exported to a GeoJSON and stored in Github for this analysis. 

Below we load our data into the R and we `st_transform` it to the **2264 coordinate reference system**, which is appropriate for Charlotte MSA and has a linear unit in feet.
...
```
## 2.3. Setup - Load Land Cover Data

Below we import our raster land cover data from the the NLCD database for our two time periods we are interested in - **2011 (t1)** and **2019 (t2)**, and clip the raster using our administrative county boundries for Charlotte, NC.
...
## 2.4. Engineering - Detecting Land Cover Conversion
