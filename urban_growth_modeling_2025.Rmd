...
```{r}
charlotteMSA <- 
  st_read("https://github.com/austinrs25/CPLN-Urban-Growth-Modeling/blob/main/charlotteMSA.shp") %>%
  st_transform(2264)
```
...
charlottePop11 <- get_acs(geography = "tract", 
                        ...
                        geometry = TRUE, 
                        output = "wide",
                        county=c("Mecklenburg","Union","Cabarrus","Gaston")) %>%
...
```{r, warning = FALSE, message = FALSE}
fishnetPopulation11 <-
  st_interpolate_aw(charlottePop11["pop_2011"], 
                    charlotteMSA_fishnet, 
                    extensive=TRUE) %>%
  st_centroid() %>%
  st_join(charlotteMSA_fishnet, ., join = st_intersects) %>%
...
```{r}
fishnetPopulation19 <-
  st_interpolate_aw(charlottePop19["pop_2019"],
                    charlotteMSA_fishnet, 
                    extensive=TRUE) %>%
  st_centroid() %>%
  st_join(charlotteMSA_fishnet, ., join = st_intersects) %>%
